...
class Phaser(pl.LightningModule):
    def __init__(self, args):
        super().__init__()
        self.model = phaser.Phaser(sample_rate=args.sample_rate,
    ...
    def test_step(self, batch, batch_idx):
        ...
        self.log("test_loss_mrsl", loss_mrsl, on_epoch=True, prog_bar=False, logger=True)
        return y_hat
...
if __name__ == '__main__':
    ...
    start = int(60 * sample_rate) - train_seq_length # custom dataset contains 60s of chirp signal (training data) followed by test audio

    train_loader = DataLoader(dataset=ds.SequenceDataset(data=audio_data,
                                                         sequence_length=train_seq_length,
                                                         start=start),
                              pin_memory=pin_memory,
                              num_workers=num_workers)
    test_loader = DataLoader(dataset=ds.SequenceDataset(data=audio_data,
                                                        sequence_length=test_seq_length+train_seq_length,
                                                        start=start)) # must be same as train loader for LFO phase consistency
    ...
    if args.checkpoint_path == '':
        ...
        model = Phaser.load_from_checkpoint(args.checkpoint_path, sample_rate=sample_rate)
